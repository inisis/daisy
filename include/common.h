...
public:
    ...
    PPM()
    {
        //std::cout << "PPM constructed" << std::endl;
    };

    void init(int Channel, int Width, int Height)
    {
        C = Channel;
        H = Height;
        W = Width;
        buffer = new uint8_t[C * H * W];
    };
...
inline void readPPMFile(const std::string& filename, samplesCommon::PPM& ppm)
{
...
inline void writePPMFileWithBBox(const std::string& filename, PPM& ppm, const BBox& bbox)
{
    ...
    const int x1 = std::min(std::max(0, round(int(bbox.x1))), ppm.W - 1);
    const int x2 = std::min(std::max(0, round(int(bbox.x2))), ppm.W - 1);
    const int y1 = std::min(std::max(0, round(int(bbox.y1))), ppm.H - 1);
    const int y2 = std::min(std::max(0, round(int(bbox.y2))), ppm.H - 1);
    for (int x = x1; x <= x2; ++x)
    {
        // bbox top border
