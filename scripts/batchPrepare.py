...
try:
	...
	raise ImportError("""ERROR: Failed to import module ({})
pip install commentjson""".format(err))

parser = argparse.ArgumentParser()
parser.add_argument('--jsonFile', required=True, help='Json file')
...
config = commentjson.load(jsonfile)

height = int(config['Height'])
width = int(config['Width'])
mean = config['Mean']
NUM_BATCHES = int(config['NB_CAL_BATCHES'])
NUM_PER_BATCH = int(config['CAL_BATCH_SIZE'])
NUM_CALIBRATION_IMAGES = NUM_BATCHES * NUM_PER_BATCH

print(height)
print(NUM_CALIBRATION_IMAGES)
print(float(mean[0]))
# images to test
imgs = []
...
for i in range(NUM_BATCHES):
	...
	for j in range(NUM_PER_BATCH):
		...
		in_ = in_[:,:,::-1]
		in_-= np.array((float(mean[0]), float(mean[1]), float(mean[2])))
		## in_ *= 1.0/256;
